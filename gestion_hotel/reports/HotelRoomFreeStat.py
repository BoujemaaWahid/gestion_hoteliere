...
class RoomReservationSummary(models.Model):
    _name = 'room.stat.free'
    _description = 'Room free'
    ...
    date_to = fields.Datetime('Date To',default=datetime.today()+timedelta(days=8))

    date_from1 = fields.Date('Date From',default=date.today())
    date_to1 = fields.Date('Date To',default=date.today()+timedelta(days=8))
    summary_header = fields.Text('Summary Header')
    room_summary = fields.Text('Room Summary')
    roomsToClean = fields.Many2many('hotel.room',string="Rooms")

    @api.multi
    def getReport(self):
        print(self.summary_header)
        print(self.room_summary)
        data = {
            'ids': self.ids,
            'model': self._name,
            'form': {
                'header': self.summary_header,
                'body':self.room_summary,
                'date_debut':self.date_from1,
                'date_fin':self.date_to1
            },
        }

        return self.env.ref('gestion_hotel.hotel_stat_free_rooms_report_file').report_action(self, data=data)
    ...
    @api.onchange('date_from1','date_to1')
    def get_room_summary(self):

        dd15 = datetime.combine(self.date_from1,datetime.min.time())
        dd16 = datetime.combine(self.date_to1,datetime.min.time())
        self.date_from = dd15
        self.date_to = dd16
        array = []
        res = {}
        ...
        if self.date_from and self.date_to:
            ...
            for room in room_ids:
                            ...
                            for res_room in reservline_ids:
                                ...
                                if room_list_stats:
                                    ...
                                    for rlist in room_list_stats:
                                        ...
                                        reservationsModule = self.env['hotel.reservation'].search(
                                            [('id', '=', res_room.id)])
                ...
                query = "select count(*) from hotel_reservation_hotel_room_rel Rl, hotel_reservation Hr where Rl.hotel_reservation_id = Hr.id and Rl.hotel_room_id = {} and ( CAST(Hr.checkout as date) = '{}' ) and Hr.state='consuming'".format(
                    room.id, DT)
                ...
                if r[0][0] != 0:
                    room_detail.update({'isOut': True})
                    array.append(room.id)
                else:
                    room_detail.update({'isOut': False})
